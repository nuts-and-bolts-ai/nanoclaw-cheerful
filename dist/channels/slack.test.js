...
// Mock channel-routing
vi.mock('../channel-routing.js', () => ({
    resolveChannelFolder: vi.fn().mockReturnValue('slack-test-channel'),
}));
const appRef = vi.hoisted(() => ({ current: null }));
vi.mock('@slack/bolt', () => ({
    App: class MockApp {
        ...
        client = {
            ...
            conversations: {
                ...
                info: vi.fn().mockResolvedValue({
                    channel: { id: 'C_NEW_CHAN', name: 'test-channel' },
                }),
            },
            users: {
                info: vi.fn().mockResolvedValue({
...
async function triggerMessageEvent(event) {
...
describe('SlackChannel', () => {
    ...
    describe('message handling', () => {
        it('delivers message for registered channel', async () => {
            ...
            expect(opts.onChatMetadata).toHaveBeenCalledWith('slack:C0123456789', expect.any(String), undefined, 'slack', true);
        ...
        it('only emits metadata for unregistered channels without bot mention', async () => {
            const opts = createTestOpts();
            const channel = new SlackChannel(opts);
            await channel.connect();
            const event = createMessageEvent({ channel: 'C9999999999' });
            await triggerMessageEvent(event);
            expect(opts.onChatMetadata).toHaveBeenCalledWith('slack:C9999999999', expect.any(String), undefined, 'slack', true);
        ...
        it('identifies IM channel type as non-group', async () => {
            ...
            await triggerMessageEvent(event);
            expect(opts.onChatMetadata).toHaveBeenCalledWith('slack:D0123456789', expect.any(String), undefined, 'slack', false);
        });
    ...
    describe('auto-registration on mention', () => {
        it('auto-registers channel when bot is mentioned in unregistered channel', async () => {
            ...
            const channel = new SlackChannel(opts);
            await channel.connect();
            // Mock conversations.info for channel name lookup
            currentApp().client.conversations = {
                ...currentApp().client.conversations,
                info: vi.fn().mockResolvedValue({
                    channel: { id: 'C_NEW_CHAN', name: 'cheerful-sales' },
                }),
            };
            vi.mocked(resolveChannelFolder).mockReturnValue('cheerful');
            const event = createMessageEvent({
                channel: 'C_NEW_CHAN',
                text: 'Hey <@U_BOT_123> what do you think?',
                user: 'U_USER_456',
            ...
            await triggerMessageEvent(event);
            expect(onRegisterGroup).toHaveBeenCalledWith('slack:C_NEW_CHAN', expect.objectContaining({
                name: 'cheerful-sales',
                folder: 'cheerful',
                requiresTrigger: true,
            }));
            // Message should also be delivered
            expect(opts.onMessage).toHaveBeenCalled();
        });
        it('does not auto-register when bot is not mentioned', async () => {
            const onRegisterGroup = vi.fn();
            const opts = createTestOpts({
                registeredGroups: vi.fn(() => ({})),
                onRegisterGroup,
        ...
        it('skips auto-registration for bot messages', async () => {
            const onRegisterGroup = vi.fn();
            const opts = createTestOpts({
                registeredGroups: vi.fn(() => ({})),
                onRegisterGroup,
    ...
    describe('sendMessage', () => {
        ...
        it('replies in thread when triggered by @mention in requiresTrigger channel', async () => {
            const opts = createTestOpts({
                registeredGroups: vi.fn(() => ({
                    'slack:C0123456789': {
                        name: 'Test Channel',
            ...
            await triggerMessageEvent(event);
            // Reply should be threaded under the trigger message
            await channel.sendMessage('slack:C0123456789', 'Sure!');
            expect(currentApp().client.chat.postMessage).toHaveBeenCalledWith({
                channel: 'C0123456789',
                text: 'Sure!',
        ...
        it('does not thread replies for non-requiresTrigger channels', async () => {
            const opts = createTestOpts({
                registeredGroups: vi.fn(() => ({
                    'slack:C0123456789': {
                        name: 'Assistant',
            ...
            const channel = new SlackChannel(opts);
            await channel.connect();
            // Message with @mention in a non-requiresTrigger channel
            const event = createMessageEvent({
                text: 'Hey <@U_BOT_123> help me',
                ts: '1704067200.000000',
                user: 'U_USER_456',
    ...
    describe('constructor', () => {
        it('throws when SLACK_BOT_TOKEN is missing', () => {
            ...
            expect(() => new SlackChannel(createTestOpts())).toThrow('SLACK_BOT_TOKEN and SLACK_APP_TOKEN must be set in .env');
        });
    ...
    describe('syncChannelMetadata pagination', () => {
        it('paginates through multiple pages of channels', async () => {
            ...
            expect(currentApp().client.conversations.list).toHaveBeenNthCalledWith(2, expect.objectContaining({ cursor: 'cursor_page2' }));
