...
function createSchema(database) {
  ...
  `);
    ...
    // Migration: remove UNIQUE constraint from folder column
    // (multiple channels can now share a folder via channel routing)
    try {
      ...
      `);
            logger.info('Migrated registered_groups: removed UNIQUE constraint from folder');
        }
    }
    catch {
...
export function getMessagesSince(chatJid, sinceTimestamp, botPrefix, threadTs) {
    // Filter bot messages using both the is_bot_message flag AND the content
    // prefix as a backstop for messages written before the migration ran.
    ...
    ORDER BY timestamp
  `;
    const params = [chatJid, sinceTimestamp, `${botPrefix}:%`];
