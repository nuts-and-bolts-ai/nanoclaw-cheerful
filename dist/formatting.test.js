...
describe('formatMessages', () => {
    ...
    it('marks trigger messages when triggerPattern is provided', () => {
        const msgs = [
            makeMsg({ content: 'just chatting', sender_name: 'Alice' }),
            makeMsg({ content: `@${ASSISTANT_NAME} help me`, sender_name: 'Bob' }),
            makeMsg({ content: 'more context', sender_name: 'Alice' }),
        ];
        const result = formatMessages(msgs, TRIGGER_PATTERN);
        expect(result).toContain('<message sender="Alice"');
        expect(result).toContain(`<trigger sender="Bob"`);
        expect(result).toContain('</trigger>');
        expect(result).toContain('>more context</message>');
    });
    it('does not mark triggers when no pattern is provided', () => {
        const msgs = [
            makeMsg({ content: `@${ASSISTANT_NAME} help me`, sender_name: 'Bob' }),
        ];
        const result = formatMessages(msgs);
        expect(result).not.toContain('<trigger');
        expect(result).toContain('<message sender="Bob"');
    });
});
// --- TRIGGER_PATTERN ---
