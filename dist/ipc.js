                                    ...
                                    try {
                                        ...
                                        if (data.type === 'message' && data.chatJid && data.text) {
                                            ...
                                            if (isMain ||
                                                (targetGroup && targetGroup.folder === sourceGroup)) {
                                                await deps.sendMessage(data.chatJid, data.text);
                                                logger.info({ chatJid: data.chatJid, sourceGroup, threadTs }, 'IPC thread message sent');
                                            }
                                            else {
                                                logger.warn({ chatJid: data.chatJid, sourceGroup, threadTs }, 'Unauthorized IPC thread message attempt blocked');
                                            }
                                        }
                                        fs.unlinkSync(filePath);
                                    }
                                    catch (err) {
                                        logger.error({ file, sourceGroup, threadTs, err }, 'Error processing IPC thread message');
                                        const errorDir = path.join(ipcBaseDir, 'errors');
                                        fs.mkdirSync(errorDir, { recursive: true });
                                        fs.renameSync(filePath, path.join(errorDir, `${sourceGroup}-thread-${threadTs}-${file}`));
                                    }
                                }
                            }
                        }
                        catch (err) {
                            logger.error({ err, sourceGroup, threadTs }, 'Error reading IPC thread messages directory');
                        }
                            ...
                            if (fs.existsSync(threadTasksDir)) {
                                ...
                                for (const file of taskFiles) {
                                    ...
                                    try {
                                        ...
                                        logger.error({ file, sourceGroup, threadTs, err }, 'Error processing IPC thread task');
                                        const errorDir = path.join(ipcBaseDir, 'errors');
                                        fs.mkdirSync(errorDir, { recursive: true });
                                        fs.renameSync(filePath, path.join(errorDir, `${sourceGroup}-thread-${threadTs}-${file}`));
                                    }
