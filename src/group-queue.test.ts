...
describe('GroupQueue', () => {
  ...
  describe('folder-level locking', () => {
    it('blocks second group on same folder until first completes', async () => {
      const executionOrder: string[] = [];
      let resolveFirst: () => void;
      ...
      const processMessages = vi.fn(async (groupJid: string) => {
        executionOrder.push(`start:${groupJid}`);
        if (groupJid === 'group-a') {
          await firstBlocked;
        }
        executionOrder.push(`end:${groupJid}`);
        return true;
      });

      queue.setProcessMessagesFn(processMessages);
      queue.setFolderResolver((jid) => {
        // Both groups share 'shared-folder'
        if (jid === 'group-a' || jid === 'group-b') return 'shared-folder';
        return jid;
      });
      ...
      queue.enqueueMessageCheck('group-b');
      await vi.advanceTimersByTimeAsync(0);

      // group-b should NOT have started yet
      expect(executionOrder).toEqual(['start:group-a']);

      // Release group-a
      resolveFirst!();
      await vi.advanceTimersByTimeAsync(0);
      // Need to let microtasks settle
      await vi.advanceTimersByTimeAsync(0);

      expect(executionOrder).toContain('end:group-a');
      expect(executionOrder).toContain('start:group-b');
    });

    it('allows groups on different folders to run concurrently', async () => {
      let concurrentCount = 0;
      let maxConcurrent = 0;
      ...
      queue.setProcessMessagesFn(processMessages);
      queue.setFolderResolver((jid) => jid); // each group gets its own folder
