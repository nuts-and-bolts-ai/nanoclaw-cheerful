...
export class SlackChannel implements Channel {
  ...
  private threadTargets = new Map<string, string>(); // jid -> ts to reply in thread
  private activeThreads = new Map<string, Set<string>>(); // jid -> set of thread parent ts where bot is active
  ...
  private setupEventHandlers(): void {
    ...
    this.app.event('message', async ({ event }) => {
      ...
      logger.info(
        { jid, text: msg.text?.slice(0, 50), channel: msg.channel },
      ...
      if (!groups[jid]) {
        ...
        if (!isBotMentioned || !this.opts.onRegisterGroup) {
          logger.debug(
            { jid },
            'Slack message from unregistered JID without bot mention, skipping',
          );
          return;
        }
        ...
        if (!channelName) {
          logger.warn(
            { jid },
            'Could not resolve channel name for auto-registration',
          );
          return;
        }
        ...
        logger.info(
          ...
          'Auto-registering Slack channel on @mention',
        );
      ...
      const isBotMentioned = !!(this.botUserId && !isBotMessage && content.includes(`<@${this.botUserId}>`));
  ...
  private async resolveChannelName(
    ...
    try {
      ...
      return (result.channel as { name?: string })?.name;
    } catch (err) {
